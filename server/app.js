        ...
        if (!email || !password) {
            ...
            if (!user) {
                ...
                if (!validateUser) {
                    ...
                    jwt.sign(payload, JWT_SECRET_KEY, { expiresIn: '1d' }, async (err, token) => {
                        user.token = token;
                        await user.save();
...
app.get('/api/conversations/:userId', async(req, res) => {
    try {
        ...
        const conversationUserData= Promise.all(conversations.map(async(conversation) => {
            ...
            return { user: user? {receiverId: user._id, email: user.email, fullName: user.fullName} : null, conversationId: conversation._id}
        }))
...
app.post('/api/message', async (req, res) => {
    try {
        ...
        const newMessage = new Messages({ conversationId, senderId, message }); // Use 'message' instead of 'messages'
        await newMessage.save();
...
app.get('/api/message/:conversationId', async(req, res) => {
    try {
        ...
        if(conversationId === 'new') {
            const checkConversations= await Conversations.find({ members: {$all: [req.query.senderId, req.query.receiverId]}})
            if(checkConversations.length>0){
                checkMessages(checkConversations[0]._id);
            }
            else {
